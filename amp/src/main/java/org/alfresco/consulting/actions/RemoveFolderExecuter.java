...
public class RemoveFolderExecuter extends ActionExecuterAbstractBase{
	...
	/**
	 * Used to exclude directories from where this action will execute. such as template folder.
	 */
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		...
		if (exceptionList == null
			...
			if (forceDeletion != null && forceDeletion)
				...
				if (logger.isDebugEnabled()) {
					for (FileInfo child : fileFolderService.listFiles(owningNodeRef)) {
						logger.debug("Children: " + child.getNodeRef() + " - " +child.getName()  ); 
					}
				}
				int childrenCount = fileFolderService.listFiles(owningNodeRef).size();
				logger.debug("Children Count for: " + owningNodeRef + " : " + childrenCount);
				if (childrenCount > 0) {
					String msg = "Deletion failed.Folder {"
							+ owningNodeRef + "} is not empty ";
